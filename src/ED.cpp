...
ED::ED(Mat _srcImage, const std::vector<cv::KeyPoint>& keypoints, GradientOperator _op, int _gradThresh, int _anchorThresh,int _scanInterval, int _minPathLen ,double _sigma, bool _sumFlag)
{	
	// Check parameters for sanity
	if (_gradThresh < 1) _gradThresh = 1;
	if (_anchorThresh < 0) _anchorThresh = 0;
...
{	
	...
	for (int i = 1; i<height - 1; i++) {
		for (int j = 1; j<width - 1; j++) {
			...
			int index = i*width + j;
			// gradImg[index] = sum;
			gradImg[index] = (255-srcImg[index]);
			// gradImg[index] = (207-smoothImg[index]);
			// dirImg[index] = EDGE_HORIZONTAL;
			if (gradImg[index] >= anchorThresh) {
				if (gx >= gy) dirImg[index] = EDGE_VERTICAL;
				else          dirImg[index] = EDGE_HORIZONTAL;
			} //end-if
		} // end-for
	} // end-for
	// gradImage = equalizeHist16SC1(gradImage);
}
...
void ED::JoinAnchorPointsUsingSortedAnchors()
{
	...
	StartOfWhile:
		while (top >= 0) {
			...
			pixels[len].y = r;
			pixels[len].x = c;
			chains[noChains].isAnchor = isPointAnAnchor(pixels[len]);
			...
			if (dir == LEFT) {
				...
				while (dirImg[r*width + c] == EDGE_HORIZONTAL) {
					...
					len++;
					chainLen++;
					// cout<<"RIGHT-while"<<endl;
				} //end-while
				// cout<<"RIGHT-while-end"<<endl;
				stack[++top].r = r;
				stack[top].c = c;
				...
				while (dirImg[r*width + c] == EDGE_VERTICAL) {
					...
					len++;
					chainLen++;
					// cout<<"DOWN-while"<<endl;
				} //end-while
				// cout<<"DOWN-while-end"<<endl;
				stack[++top].r = r;
				stack[top].c = c;
				stack[top].dir = RIGHT;
				stack[top].parent = noChains;
		...
		if (len - duplicatePixelCount < minPathLen) {
			...
			// AdjustTreeAndCheckParent(chains, chains[0].children[1], 0);
...
void AdjustTreeAndCheckParent(Chain *chains, int root, int parent) {
    if (root == -1) return;

    bool isLeaf = true;
    for (int i = 0; i < 2; ++i) {
        int child = chains[root].children[i];
        if (child != -1) {
            AdjustTreeAndCheckParent(chains, child, root);
            if (chains[child].children[0] != -1 || chains[child].children[1] != -1 || chains[child].isAnchor) {
                isLeaf = false; // 如果子节点是锚点或者非叶子节点，则当前节点不是叶子节点
            }
        }
    }

    if (isLeaf && !chains[root].isAnchor) {
        // 如果当前节点是非锚点的叶子节点，通知其父节点断开连接
        if (parent != -1) { // 检查是否有有效的父节点
            if (chains[parent].children[0] == root) chains[parent].children[0] = -1;
            else if (chains[parent].children[1] == root) chains[parent].children[1] = -1;
            // 递归向上检查父节点
            AdjustTreeAndCheckParent(chains, parent, chains[parent].parent);
        }
    }
}